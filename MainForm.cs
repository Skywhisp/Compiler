...
namespace Compiler
{
    public partial class MainForm : Form
    {
        ...
        private void btnFindWordsStartingWithF_Click(object sender, EventArgs e)
        {
            string inputText = richTextBox1.Text;
            Regex regexF = new Regex(@"\b[fF]\w*\b");
            MatchCollection matches = regexF.Matches(inputText);
            DisplayMatches(matches);
        }

        private void btnFindMultilineComments_Click(object sender, EventArgs e)
        {
            string inputText = richTextBox1.Text;
            Regex regexComments = new Regex(@"(?s)\""\"".*?\""\""");
            MatchCollection matches = regexComments.Matches(inputText);
            DisplayMatches(matches);
        }
        ...
        {
            string inputText = richTextBox1.Text;
            Regex regexPass = new Regex(@"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[#?!@$%^&*_\-|]).{10,}$");
            bool isValid = regexPass.IsMatch(inputText);
            richTextBox2.Text = isValid ? "Пароль подходит" : "Пароль не подходит";
        }

        private void DisplayMatches(MatchCollection matches)
        {
            StringBuilder result = new StringBuilder();
            foreach (Match match in matches)
            {
                result.AppendLine($"Найден '{match.Value}' на позиции {match.Index}");
            }
